...
_via_data.prototype.attribute_update_options_from_csv = function (
	aid,
	options_csv
) {
	return new Promise(
			...
			try {
				if (this.store.file.hasOwnProperty(fid)) {
					if (name === "src") {
						...
						if (name === "loc_prefix") {
							this.store.config.file.loc_prefix[
								this.store.file[fid].loc
							] = value;
						} else {
		...
		function (ok_callback, err_callback) {
			try {
				...
				for (var i = 0; i < filelist.length; ++i) {
					...
					//	//upload image to remort server-post request; only if the store-location type is remote
					if (filelist[i].store === _VIA_FILE_STORE.REMOTE) {
						console.log("remote", filelist);
						// alert(image);
			...
			try {
				...
				for (var metadata_type in metadata_type_anchor_id_map) {
					...
					for (var aid in via2["_via_attributes"][metadata_type]) {
						...
						var atype_text = via2["_via_attributes"][metadata_type][aid][
							"type"
						].toUpperCase();

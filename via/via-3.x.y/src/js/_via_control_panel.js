...
_via_control_panel.prototype._init = function (type) {
	...
	if (document.getElementById("micon_zoomin")) {
		var zoom = _via_util_get_svg_button(
			"micon_zoomin",
...
_via_control_panel.prototype._add_view_manager_tools = function () {
	...
	var url_add = _via_util_get_svg_button(
		"micon_upload",
		"upload the image file",
		"test_kkkkmedia"
	);
	url_add.addEventListener(
		"click",
		function () {
			var if_local = false;
			this.via.vm._on_add_media_local(if_local);
			console.log("hiiii");
		}.bind(this)
	);

	this.c.appendChild(url_add);
...
_via_control_panel.prototype._add_project_share_tools = function () {
	if (this.via.s) {
		var share = _via_util_get_svg_button(
			"micon_share",
			"Information about sharing this VIA project with others for collaborative annotation"
		);
					...
					function (ok) {
						try {
							...
							var local_rev_timestamp = new Date(
								parseInt(this.via.d.store.project.rev_timestamp)
							);

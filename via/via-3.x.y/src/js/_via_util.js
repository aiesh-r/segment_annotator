...
function _via_util_page_gather_user_input() {
	...
	for (var i = 0; i < select_list.length; ++i) {
		user_input[select_list[i].id] =
			select_list[i].options[select_list[i].selectedIndex].value;
	}
		...
		switch (input_list[i].type) {
			...
			case "select":
				if (input_list[i].selectedIndex === -1) {
					...
					user_input[input_list[i].id] =
						input_list[i].options[input_list[i].selectedIndex];
				}
				break;
			case "text":
			case "password":
			...
			default:
				console.warn(
					"Input type=" + input_list[i].type + " not yet implemented!"
				);
		}
	}
...
function _via_hh_mm_ss_ms_to_seconds(hh_mm_ss_ms) {
	...
	switch (split2.length) {
		case 3:
			sec =
				parseInt(split2[0]) * 60 * 60 +
				parseInt(split2[1]) * 60 +
				parseInt(split2[2]);
			break;
		case 2:
